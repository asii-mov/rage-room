"""Rage Room Remote CLI — manage DigitalOcean sandbox VMs."""
...
from rage_room_remote.config import (
    load_config,
    save_config,
    validate_config,
)

TAG_PREFIX = "rage-room-sandbox"
...
def get_client(config: dict) -> DOClient:
    ...
    if errors:
        ...
        click.echo("\nRun: rage-room-remote config", err=True)
        sys.exit(1)
...
def ensure_ssh_key(client: DOClient, config: dict) -> int:
    ...
    # Upload new key
    key_name = f"rage-room-{config.get('username', 'dev')}"
...
@click.group()
def cli():
    """Rage Room Remote — manage DigitalOcean sandbox VMs."""
    pass
...
def config():
    ...
    click.echo("Rage Room Remote Configuration")
    click.echo("=" * 40)
    ...
    current["container_image"] = click.prompt(
        ...
        default=current.get("container_image", "rage-room-sandbox:latest"),
    )
    ...
    for d in droplets:
        ...
        for net in d.get("networks", {}).get("v4", []):
            if net["type"] == "public":
                ip = net["ip_address"]
                break
        display_name = d['name'].removeprefix("rage-room-")
        click.echo(
            f"{display_name:<25} {ip:<18} {d['region']['slug']:<8} "
...
def hibernate(name: str):
    ...
    droplet_id = droplet["id"]
    snapshot_name = f"rage-room-{name}-{int(time.time())}"
    ...
    click.echo(f"Sandbox '{name}' hibernated. Wake with: rage-room-remote wake {name}")
...
def share(name: str, users: str):
    ...
    Example: rage-room-remote share my-sandbox user1,user2,user3
    """
    config = load_config()
    client = get_client(config)
