#!/usr/bin/env python3
"""Post-install configuration for Rage Room Sandbox.
...
def fix_directory_ownership():
    """Check ownership of mounted volumes and warn if mismatched."""
    uid = os.getuid()
        ...
        if dir_path.exists():
            try:
                ...
                if stat_info.st_uid != uid:
                    print(
                        f"[post_install] Warning: {dir_path} owned by uid {stat_info.st_uid}, "
                        f"expected {uid}. Volume may need manual fix.",
                        file=sys.stderr,
                    )
            except PermissionError as e:
                print(
                    f"[post_install] Warning: Cannot stat {dir_path}: {e}",
                    file=sys.stderr,
                )
